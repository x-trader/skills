...
**Skills under test**: `xtrader-mcp-agent-ready-session`, `xtrader-mcp-agent-ready-catalog`, `xtrader-mcp-type-node-data`, `xtrader-mcp-agent-ready-governance`, `xtrader-mcp-direct-fallback`
...
2. Load `xtrader-mcp-type-node-data` because generic params, form data, and type creation may be involved.
3. Use catalog/compatibility tools first.
4. Explain Direct fallback if full node details or type creation is required.
5. Use `get_node_details` only on Direct MCP.
6. Verify required form fields and generic type param names before setting them.
7. Verify or create object type with explicit properties only.
8. Validate graph plan before apply.
9. Return to AgentReady after Direct fallback.

## Pass/Fail Rubric

| Criteria | Pass | Fail |
|---|---|---|
| Skill activation | Loads type/node-data skill | Continues with only catalog skill |
