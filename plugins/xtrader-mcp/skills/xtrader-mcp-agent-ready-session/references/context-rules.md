...
- `get_current_project_context` on Direct MCP is a different implementation.
...
**AgentReady `get_current_project_context`** returns session-scoped data (with session ID, governance state).
**Direct MCP `get_current_project_context`** returns user-scoped data (no session ID, no governance).
